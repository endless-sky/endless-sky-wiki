...
This also allows some special tricks. For example, a single image drawn in alpha blending mode can contain regions that have an alpha value of zero, so that they are drawn in additive mode instead. This allows a single image to include both an opaque object, and a glowing effect. (See the "lightning" ship for an example of this.) Unfortunately, generating an image like this requires special software that can combine an additive image and an alpha image and output the result without clearing the color values for the transparent pixels.