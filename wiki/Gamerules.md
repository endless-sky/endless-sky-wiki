...
* `"base combat width"`: a decimal rule whose value can be any number greater than or equal to 0, or "unset". Controls the maximum number of crew that can participate in a single round of boarding based on the bunk count of each ship. The smallest combat width between the attacking and defending ship is used. If this gamerule has a value of "unset", ships will always use their entire crew during boarding.
