...
* `"spawn raid fleets"`: A boolean rule that controls whether raid fleets spawn when your pirate attraction is too high. When off, the `"raid chance in system: <system>"` condition always returns 0, but the `"pirate attraction"` condition will still return non-zero values. It will also hide the piracy threat information from the player info panel. **(v. 0.11.1)**
