...
The game data in Endless Sky includes the following elements, sometimes referred to as "root-level" nodes or tokens:
  ...
  * `fleet`: a list of possible ship combinations and relative frequencies. (For examples, see [`fleets.txt`](data/fleets.txt).)
  ...
  * `government`: the characteristics of a particular government. (For examples, see [`governments.txt`](data/governments.txt).)
  * `interface`: the layout of a user interface element. (For examples, see [`interfaces.txt`](data/interfaces.txt).)
  ...
  * `phrase`: a rule for constructing random phrases, e.g. for ship names, hail messages, and the name used for a spaceport news message. (See [`hails.txt`](data/hails.txt) or [`names.txt`](data/names.txt) for examples.)
  ...
  * `trade`: a list of commodity names and prices. (For examples, see [`trade.txt`](data/trade.txt).)
